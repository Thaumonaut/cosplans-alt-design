<script lang="ts">
  /**
   * Team Moodboard Redirect
   * Route: /teams/[id]/moodboard
   * Task: T-005
   *
   * Redirects to the team's moodboard in the main moodboard editor.
   */
  ...
  onMount(async () => {
    try {
      const moodboard = await moodboardsService.getTeamMoodboard(teamId);

      if (moodboard) {
        goto(`/moodboard/${moodboard.id}`, { replaceState: true });
      } else {
        error = 'Moodboard not found for this team';
      }
    } catch (err) {
      console.error('Failed to load team moodboard:', err);
